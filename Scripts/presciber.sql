...
WITH X AS (
	...
	TOTAL_DAYS_SUPPLY,	
	CAST(TOTAL_COST AS MONEY) TOTAL_COST,
	CAST(COST_PER_DAY AS MONEY) COST_PER_DAY,
	RANK () OVER (ORDER BY COST_PER_DAY DESC) RANKING
FROM
(SELECT
	UPPER(GENERIC_NAME) GENERIC_DRUG_NAME,
	SUM(COALESCE(PR.TOTAL_DAY_SUPPLY,0)) TOTAL_DAYS_SUPPLY,
	SUM(COALESCE(PR.TOTAL_DRUG_COST,0))  TOTAL_COST,
    CASE
	WHEN SUM(COALESCE(PR.TOTAL_DAY_SUPPLY,0)) = 0 THEN 0
    ELSE (SUM(COALESCE(PR.TOTAL_DRUG_COST,0))/SUM(COALESCE(PR.TOTAL_DAY_SUPPLY,0)))*100 END COST_PER_DAY
FROM 
	PRESCRIPTION PR 
	INNER JOIN DRUG D ON UPPER(TRIM(PR.DRUG_NAME)) = UPPER(TRIM(D.DRUG_NAME))
	GROUP BY 
